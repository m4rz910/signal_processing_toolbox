...
# plt.legend(); plt.grid(); plt.xlabel('Octave Band [Hz]'); plt.ylabel('C80');
...
#         t_end = edc_filt[round(edc_filt,1)==-10].index[0]
#         octives.append((i, (t_end-t_zero)*6))
#     plt.plot(*zip(*octives),line, c=color,marker='o',markersize=8, label=r_lab);
# plt.legend(); plt.grid(); plt.xlabel('Octave Band [Hz]'); plt.ylabel('EDT [s]');
# plt.xticks(range(0,6),cuts)
...
# #    Octive Band Filter
    ...
    for i,dummy in enumerate(cuts):
        ...
        t_end = edc_filt[round(edc_filt,1)==-35].index[0] #changed from edt
        octives.append((i, (t_end-t_zero)*2)) #changed from edt
    plt.plot(*zip(*octives),line, c=color,marker='o',markersize=8, label=r_lab);
plt.legend(); plt.grid(); plt.xlabel('Octave Band [Hz]'); plt.ylabel('T30 [s]');
plt.xticks(range(0,6),cuts)

fig.savefig('./plots/final_T30.png', dpi=300, bbox_inches='tight')
